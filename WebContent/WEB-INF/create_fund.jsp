	...
	<div class="content">
		<div class="left">
			...
			<div class="login_option">
				<div class="create_accounts"><a href="customerCredential.do" style="text-decoration: none; color: #A1A9A9;">CREATE ACCOUNTS</a></div>
				<div class="deposit_money"><a href="deposit.do" style="text-decoration: none; color: #A1A9A9;">DEPOSIT MONEY</a></div>
				<div class="create_fund"><a href="createFund.do" style="text-decoration: none; color: #F9460D;">CREATE FUND</a></div>
			</div>
		</div>
				...
				<form action="createFund.do" method="POST">
					<c:forEach var="field" items="${form.visibleFields}">
						...
						<label style="color: red;">${field.error}</label>
			  			<input type="${field.type}" style="width: 300px; height:20px;" id="${field.name}" name="${field.name}" value="${field.value}" placeholder="Employee id" value=""><br><br>
					</c:forEach>
		  			<input class="button" type="submit" name="action" value="Create">
				</form>
			</div>
			<hr class="right_bottom_line" style="margin-top: 315px;">
		</div>
	</div>
</body>
</html>